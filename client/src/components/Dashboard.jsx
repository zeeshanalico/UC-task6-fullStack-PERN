...
import { createItem, changeStatusOfItem, updateItem, deleteItem,getItems } from '../api/api.js';
...
const Dashboard = ({ role: { roleName } }) => {
  ...
  const statusChangeHandler = (status) => {
    checkedItems.map(async id => {
      const response = await changeStatusOfItem(id, status);
      fetchData();
      masterCheckBox.current.checked = false;
    });
  };
  ...
  const submitHandle = async (e, action, currentItem) => {
    ...
    try {
      if (action === 'create') {
        const response = await createItem(title, duration, link, status);
        setSelectedItem(null)
        setShowCreateModal(false)
      } else if (action === 'update') {
        const response = await updateItem(id, title, duration, link);
        ...
        <table className='table'>
          <thead>
            <tr>
              ...
              {(roleName === 'ADMIN' || roleName === 'EDITOR') && <td className='text-dark'>Edit</td>}
              {(roleName === 'ADMIN') && <td className='text-dark'>Delete</td>}
            </tr>
          </thead>
          <tbody>
            {list
              ?.filter(item => item.status === status)
              ?.map((item, keyID) => <Item key={item.id} item={item} handleCheckedItems={handleCheckedItems} editHandler={editHandler} deleteHandler={deleteHandler} roleName={roleName} setShowUpdateModal={setShowUpdateModal} keyID={keyID} ref={checkBoxesRef} />)
            }
          </tbody>
        </table>
